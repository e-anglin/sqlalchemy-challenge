...
# 2. If the precipitation route is selected
...
def precipitation():
    ...
    conv_date_results = dt.datetime.strptime(date_results, '%Y-%m-%d').date()
    # Calculate the date one year from the last date in data set.
    one_year = conv_date_results - dt.timedelta(days=365)
    # Perform a query to retrieve the data and precipitation scores
    sel = [measurement.date, measurement.prcp]
    query_result = session.query(*sel).filter(measurement.date <= conv_date_results).filter(measurement.date > one_year).all()    
    session.close()
...
# 3. If the stations route is selected
...
# 5. If the start date is selected
...
# 6. If start and end dates are selected
